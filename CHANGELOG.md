...
- **Empacotamento e atualização do Desktop via Velopack (`adr/ADR-019-empacotamento-atualizacao-velopack.md`, Aceita):**
  ...
  - `App.xaml.cs`: `VelopackApp.Build().Run()` como primeira instrução do app (hook obrigatório
    do Velopack); prompt bloqueante de atualização forçada em `OnStartup`, antes de abrir a
    `MainWindow`, quando a política exige.
  ...
  - `docs/26-empacotamento-atualizacao-velopack.md`: comandos `dotnet publish`/`vpk pack`/`vpk
    ...
    sem servidor próprio), nenhum token em código.
