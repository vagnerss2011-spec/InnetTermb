...
internal sealed class StoreWinBoxCredentialResolver : IWinBoxCredentialResolver
{
    private readonly ILocalStore _store;
    private readonly IVault _vault;
    private readonly ITerminalSecurityContext _securityContext;

    public StoreWinBoxCredentialResolver(
        ILocalStore store,
        IVault vault,
    ...
    public async Task<string?> ResolvePasswordAsync(string credentialRefId, CancellationToken ct = default)
    {
        ...
        // Lifetime mínimo: `using` garante zeramento imediato após RevealString (ADR-009 §FIX-3).
